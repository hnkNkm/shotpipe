...
1. **アプリを起動** - システムトレイにアイコンが表示されます
2. **監視を開始** - トレイメニューから「Start Monitoring」を選択
3. **画像をコピー** 
   - macOS: `Cmd+Ctrl+Shift+4` でスクリーンショットをクリップボードにコピー
   - または画像ファイルをコピー
4. **自動検出** - コピーした画像が自動的にアプリに表示されます
5. **注釈を追加** - （今後実装予定）テキストや矢印で説明を追加
6. **ショートカットで出力** - （今後実装予定）
